...
//include_once 'config/acessaBanco.php';

 $connect = mysqli_connect('107.180.46.155', 'DocLab', '33622905', 'DocumentosLab');
  
$query="SELECT tipo, SITUACAO, data FROM upload INNER JOIN documento  ON upload.documento_id_documento = documento.id_documento  INNER JOIN situacao ON  situacao_ID_SITUACAO  = situacao.ID_SITUACAO where users_id_users = '$idUser'; ";
...
<head>
  <title>Historico de documentos enviados</title>
 <meta charset="utf-8">
...
<table class="ls-table  ls-table ls-table-striped ls-bg-header">
 ...
 echo" <tbody>
    <tr>
      <td><a href='' title=''>".$analise['tipo']."</a></td>
            <td>".$analise['SITUACAO']."</td>
      <td class='hidden-xs'>". $analise['data']."</td>
    </tr>
    </tbody>";
  }
  if(mysqli_fetch_array($dados) ==0){
      echo "<h2> sem informação para ser exibida</h2>";
  }
?>
</table>
