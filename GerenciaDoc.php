...
$sobAnalise = ("SELECT id_upload, tipo, SITUACAO,nome, arquivo FROM upload INNER JOIN documento  ON upload.documento_id_documento = documento.id_documento  INNER JOIN situacao ON  situacao_ID_SITUACAO  = situacao.ID_SITUACAO  INNER JOIN users  ON users_id_users = users.id_users; 
...
<a href="#" class="ls-ico-question" data-ls-module="topbarCurtain" data-target="#ls-feedback-curtain"><span>Sugestões</span></a>    </div>
  ...
  <main class="ls-main ">
    <div class="container-fluid">
      <h1 class="ls-title-intro ls-ico-docs">Relação de documentos recebidos </h1>
...
<table class="ls-table  ls-table ls-table-striped ls-bg-header">
 ...
 echo" <tbody>
    <tr>
    <td class='hidden-xs'>". $analise['id_upload']."</td>
      <td>".$analise['nome']."</td>
      <td>".$analise['tipo']."</td>
      <td class='hidden-xs'>". $analise['SITUACAO']."</td>
      <td class='hidden-xs'><a  href='/Doc/Documento/".$analise['arquivo']."' target='_blank' title='Visualizar Documento' class='ls-btn ls-ico-eye'></a></td>
    </tr>
    </tbody>";
  }?>
</table>
